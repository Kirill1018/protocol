...
namespace UdpShapesLib {
    public class Players {
        ...
        public void ProcessMessage (byte[] bytes) {
            ...
            {
                string name = reader.ReadString();
                byte red_col = reader.ReadByte(),
                    green_col = reader.ReadByte(),
                    blue_col = reader.ReadByte();
                players.Add(new Player(name, red_col, green_col,
                    blue_col).Shape.ToString(), new Player(name, red_col, green_col,
                    blue_col));
            }
        }
